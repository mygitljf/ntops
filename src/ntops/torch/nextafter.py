...
def _prefer_native(input, result_dtype, is_broadcast):
    ...
    if is_metax_device(input):
        if result_dtype == torch.float64 or is_broadcast or not input.is_contiguous():
            return True
        if result_dtype in (torch.float16, torch.bfloat16) and input.ndim > 1:
            return True
    return False
...
def nextafter(input, other, *, out=None):
    ...
    if _prefer_native(input, result_dtype, is_broadcast):
        ...
        torch.nextafter(input, other, out=out)
        return out
