...
def application(
    ...
    valid_output = (
        ...
        & (ot < output.source.shape[2])
        & (oh < output.source.shape[3])
        & (ow < output.source.shape[4])
    )

    random_base = n * random_samples.stride(0) + c * random_samples.stride(1)
    sample_t = ntl.load(random_samples.data_ptr() + random_base, mask=valid_output, other=0.0)
    sample_h = ntl.load(
    ...
    sample_w = ntl.load(
        random_samples.data_ptr() + random_base + 2 * random_samples.stride(2),
        mask=valid_output,
        other=0.0,
    )
    ...
    for kt in range(kernel_t):
        for kh in range(kernel_h):
            for kw in range(kernel_w):
                ...
                index = (it * input_h + ih) * input_w + iw
...
def premake(
    ...
    tensors = (
        Tensor(5, shape=(batch, channels, output_t, output_h, output_w), dtype=dtype),
        Tensor(5, shape=(batch, channels, output_t, output_h, output_w), dtype=ninetoothed.int64),
        Tensor(5, shape=(batch, channels, input_t, input_h, input_w), dtype=dtype, other=-float("inf")),
        Tensor(3, shape=(batch, channels, 3), dtype=dtype),
        Tensor(0, dtype=None, constexpr=True, name="input_t"),
        Tensor(0, dtype=None, constexpr=True, name="input_h"),
        Tensor(0, dtype=None, constexpr=True, name="input_w"),
        Tensor(0, dtype=None, constexpr=True, name="output_t"),
        Tensor(0, dtype=None, constexpr=True, name="output_h"),
        Tensor(0, dtype=None, constexpr=True, name="output_w"),
        Tensor(0, dtype=None, constexpr=True, name="kernel_t"),
