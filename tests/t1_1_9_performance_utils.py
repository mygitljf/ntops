...
def _assert_outputs_match(output, reference, rtol=2e-3, atol=2e-3):
    if isinstance(reference, tuple):
        ...
        if (
            len(reference) == 2
            and hasattr(reference[0], "dtype")
            and reference[0].dtype == torch.float64
            and _is_iluvatar()
        ):
            _assert_outputs_match(output[0], reference[0], rtol=rtol, atol=atol)
            assert output[1].shape == reference[1].shape
            assert output[1].dtype == reference[1].dtype
            return
