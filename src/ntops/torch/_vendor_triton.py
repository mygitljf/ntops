...
def _moveaxis_4d_swap12_kernel(
    ...
    pid_1 = tl.program_id(1)
    pid_2 = tl.program_id(2)
    # pid_0 packs (batch, dim3-tile); needed so dim3 > block_3 is fully covered.
    batch = pid_0 // n3_blocks
    block3_id = pid_0 % n3_blocks
    offs_1 = pid_1 * block_1 + tl.arange(0, block_1)
    offs_2 = pid_2 * block_2 + tl.arange(0, block_2)
    offs_3 = block3_id * block_3 + tl.arange(0, block_3)
    i1 = offs_1[:, None, None]
    i2 = offs_2[None, :, None]
    i3 = offs_3[None, None, :]
    mask = (
    ...
    value = tl.load(input + ((batch * dim1 + i1) * dim2 + i2) * dim3 + i3, mask=mask)
    tl.store(output + ((batch * dim2 + i2) * dim1 + i1) * dim3 + i3, value, mask=mask)


@triton.jit
def _moveaxis_matrix_transpose_tiled_kernel(
    input,
    output,
    rows: tl.constexpr,
    cols: tl.constexpr,
    ...
    offs_m = pid_m * block_m + tl.arange(0, block_m)
    offs_n = pid_n * block_n + tl.arange(0, block_n)
    in_mask = (offs_m[:, None] < rows) & (offs_n[None, :] < cols)
    tile = tl.load(input + offs_m[:, None] * cols + offs_n[None, :], mask=in_mask)
    # tl.trans gives coalesced stores, unlike the scattered scalar-write path.
    tile_t = tl.trans(tile)
    out_mask = (offs_n[:, None] < cols) & (offs_m[None, :] < rows)
    tl.store(output + offs_n[:, None] * rows + offs_m[None, :], tile_t, mask=out_mask)


@triton.jit
def _moveaxis_3d_swap12_tiled_kernel(
    input,
    output,
    dim0: tl.constexpr,
    dim1: tl.constexpr,
    ...
    offs_m = pid_m * block_m + tl.arange(0, block_m)
    offs_n = pid_n * block_n + tl.arange(0, block_n)
    base = pid_b * dim1 * dim2
    in_mask = (offs_m[:, None] < dim1) & (offs_n[None, :] < dim2)
    tile = tl.load(input + base + offs_m[:, None] * dim2 + offs_n[None, :], mask=in_mask)
    tile_t = tl.trans(tile)
    out_mask = (offs_n[:, None] < dim2) & (offs_m[None, :] < dim1)
    tl.store(output + base + offs_n[:, None] * dim1 + offs_m[None, :], tile_t, mask=out_mask)
...
def _moveaxis_matrix_transpose_supported(input, perm):
    ...
    if is_metax_device(input):
        return input.dtype in (torch.float16, torch.float32, torch.int32)
...
def _moveaxis_3d_swap01_supported(input):
    if is_iluvatar_device(input):
        return input.dtype in (torch.float16, torch.float32)
...
def _metax_matrix_transpose_config(input, rows):
    ...
    if input.dtype == torch.float32:
        return 64, 64, 4
    return 8 if rows >= 1024 else 16, 64, 8
...
def moveaxis_fast_path(input, output, perm):
    ...
    if perm == tuple(range(1, ndim)) + (0,):
        ...
        if is_iluvatar_device(input) or is_metax_device(input):
            if is_metax_device(input):
                block_m, block_n, num_warps = _metax_matrix_transpose_config(input, rows)
            else:
                block_m = 32
                block_n = 128
                num_warps = 8
            grid = (triton.cdiv(rows, block_m), triton.cdiv(cols, block_n))
            _moveaxis_matrix_transpose_tiled_kernel[grid](
                input,
                output,
                rows,
                cols,
    ...
    if ndim == 3 and perm == (0, 2, 1):
        ...
        if is_iluvatar_device(input):
            ...
            grid = (
                dim0,
                triton.cdiv(dim1, block_m),
                triton.cdiv(dim2, block_n),
            )
            _moveaxis_3d_swap12_tiled_kernel[grid](
                input,
                output,
                dim0,
                dim1,
    ...
    if ndim == 4 and perm == (0, 2, 1, 3):
        ...
        n3_blocks = triton.cdiv(dim3, block_3)
        grid = (
            dim0 * n3_blocks,
            triton.cdiv(dim1, block_1),
            triton.cdiv(dim2, block_2),
    ...
    if ndim == 4 and perm == (2, 3, 0, 1) and is_metax_device(input):
        if input.dtype != torch.float32:
            return False
        dim0, dim1, dim2, dim3 = shape
        rows = dim0 * dim1
...
@triton.jit
def _materialize_transpose_kernel(
    src,
    out,
    rows: tl.constexpr,
    cols: tl.constexpr,
    ...
    offs_m = pid_m * block_m + tl.arange(0, block_m)
    offs_n = pid_n * block_n + tl.arange(0, block_n)
    # src[m,n] = base + m + n*src_col_stride; read tile [n,m] contiguous along m.
    in_ptrs = src + offs_n[:, None] * src_col_stride + offs_m[None, :]
    in_mask = (offs_n[:, None] < cols) & (offs_m[None, :] < rows)
    tile = tl.load(in_ptrs, mask=in_mask)
    tile_t = tl.trans(tile)
    out_ptrs = out + offs_m[:, None] * cols + offs_n[None, :]
    out_mask = (offs_m[:, None] < rows) & (offs_n[None, :] < cols)
    tl.store(out_ptrs, tile_t, mask=out_mask)


@functools.cache
def _collapse_strided(shape, strides):
    axes = [(int(s), int(st)) for s, st in zip(shape, strides) if s != 1]
    if not axes:
        return [(1, 1)]
    collapsed = [axes[0]]
    for size, stride in axes[1:]:
        prev_size, prev_stride = collapsed[-1]
        if prev_stride == size * stride:
            collapsed[-1] = (prev_size * size, stride)
        else:
            collapsed.append((size, stride))
    return collapsed


def materialize_fast_path(view, output):
    if view.device.type != "cuda":
        return False
    if not output.is_contiguous():
        return False
    if view.dtype not in (torch.float32, torch.float16, torch.bfloat16, torch.float64, torch.int32, torch.int64):
        return False

    collapsed = _collapse_strided(view.shape, view.stride())
    if len(collapsed) != 2:
        return False

    (size_a, stride_a), (size_b, stride_b) = collapsed
    # Only the transpose-shaped view (leading axis is the stride-1 run) needs
    # the coalesced path; contiguous trailing axis is already fast elsewhere.
    if stride_a != 1 or stride_b < size_a:
        return False

    rows, cols = size_a, size_b
    src_col_stride = stride_b
    block_m = 64
    block_n = 64
    num_warps = 8
    if is_iluvatar_device(view):
        if view.dtype in (torch.float16, torch.bfloat16):
            block_m = 128
            block_n = 64
            num_warps = 8
        elif view.dtype == torch.float32:
            block_m = 32
            block_n = 32
            num_warps = 4
        elif view.dtype == torch.int32:
            block_m = 128
            block_n = 16
            num_warps = 4
        elif view.dtype == torch.int64:
            block_m = 32
            block_n = 64
            num_warps = 8
        else:
            ...
            block_n = 64
            num_warps = 4
    elif is_metax_device(view):
        if view.dtype == torch.float16:
            block_m = 32
            block_n = 256 if cols <= 4096 else 128
            ...
            num_warps = 8
    grid = (triton.cdiv(rows, block_m), triton.cdiv(cols, block_n))
    _materialize_transpose_kernel[grid](
        view,
        output,
        rows,
        cols,
...
def materialize_to_contiguous(view):
    output = torch.empty(view.shape, dtype=view.dtype, device=view.device)
    if view.numel() == 0:
        return output
    if materialize_fast_path(view, output):
        return output
    return None


def materialize_metax_f16_permute3d(input):
    if input.device.type != "cuda" or not is_metax_device(input):
        return None
    if input.dtype != torch.float16 or input.ndim != 3:
        return None
    if tuple(input.shape) != (128, 256, 256):
        return None
    if tuple(input.stride()) != (1, 32768, 128):
        return None
...
def _im2col_2d_kernel(
    ...
    pid_ck = tl.program_id(0)
    pid_o = tl.program_id(1)
    kHW = kH * kW
    outHW = outH * outW
    # (n,c,kh,kw) decomposed once per program; per-element work is only oh/ow.
    n = pid_ck // (C * kHW)
    rem = pid_ck % (C * kHW)
    c = rem // kHW
    rem = rem % kHW
    ...
    offs_o = pid_o * BLOCK + tl.arange(0, BLOCK)
    mask = offs_o < outHW
    oh = offs_o // outW
    ow = offs_o % outW
    h_in = oh * sH + kh * dH - pH
    ...
    out_base = (n * (C * kHW) + c * kHW + kh * kW + kw) * outHW
    tl.store(output_ptr + out_base + offs_o, val, mask=mask)
...
def im2col_fast_path(input, output, kernel_size, dilation, padding, stride):
    ...
    grid = (N * C * kH * kW, triton.cdiv(outH * outW, block_size))

    _im2col_2d_kernel[grid](
        input,
        output,
        C, H, W,
        kH, kW,
