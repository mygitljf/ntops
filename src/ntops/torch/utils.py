...
def _dtype_is_floating_point(dtype):
    # torch.dtype exposes an is_floating_point bool property; infinicore.dtype
    # (used when InfiniCore rebinds the torch global) does not, so fall back to name.
    is_fp = getattr(dtype, "is_floating_point", None)
    if isinstance(is_fp, bool):
        return is_fp
    name = str(dtype).rsplit(".", 1)[-1]
    return name in ("float16", "bfloat16", "float32", "float64", "half", "double", "float")


def _cast_dtype(tensor, dtype):
    # infinicore.Tensor.to() only accepts a Device (no dtype overload), so skip
    # the cast when the tensor already has the target dtype (the common path).
