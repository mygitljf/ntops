...
def test_moveaxis_4d_swap12_large_lastdim(dtype, shape):
    """[case 73] 4D swap of axes 1<->2 where last dim exceeds the kernel tile."""
    if dtype.is_floating_point:
        input = torch.randn(shape, dtype=dtype, device="cuda")
    else:
        input = torch.randint(0, 100, shape, dtype=dtype, device="cuda")

    output = ntops.torch.moveaxis(input, 1, 2)
    reference = torch.moveaxis(input, 1, 2)
    _assert_equal(output, reference)
